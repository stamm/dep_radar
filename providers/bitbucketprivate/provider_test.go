...
<body>
	...
	project, err := prov.getProject(context.Background(), dep_radar.Pkg("godep.example.com/app"))
	require.NoError(err)
...
func TestBBRepo_SetProject_DontSetTwice(t *testing.T) {
	...
	prov.mapProject[dep_radar.Pkg("godep.example.com/app")] = "a"
	project, err := prov.getProject(context.Background(), dep_radar.Pkg("godep.example.com/app"))
	require.NoError(err)
...
func TestBBRepo_WithDep(t *testing.T) {
	...
	pkg := dep_radar.Pkg("godep.example.com/app")
